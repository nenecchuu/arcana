...
//Generate snowflake base on https://github.com/bwmarrin/snowflake for the detail
import (
	"encoding/base64"
	"encoding/binary"
	"errors"
...
var (
	...
	mu        sync.Mutex
	podMax    int64 = -1 ^ (-1 << PODBits)
	podMask         = podMax << StepBits
	stepMask  int64 = -1 ^ (-1 << StepBits)
	timeShift       = PODBits + StepBits
	podShift        = StepBits
)
...
// A POD struct holds the basic information needed for a snowflake generator
// node
type POD struct {
	mu    sync.Mutex
	epoch time.Time
...
// An SnowflakeID is a custom type used for a snowflake ID.  This is used so we can
// attach methods onto the ID.
type SnowflakeID int64

// NewPOD returns a new snowflake pod that can be used to generate snowflake
// IDs
func NewPOD(o *SnowflakeOpts) (*POD, error) {
	// re-calc in case custom PODBits or StepBits were set
	...
	n := POD{}
	n.pod = o.POD
	n.podMax = -1 ^ (-1 << PODBits)
	n.podMask = n.podMax << StepBits
	n.stepMask = -1 ^ (-1 << StepBits)
	n.timeShift = PODBits + StepBits
	n.podShift = StepBits

	if n.pod < 0 || n.pod > n.podMax {
		return nil, errors.New("POD number must be between 0 and " + strconv.FormatInt(n.podMax, 10))
	}
...
// - Make sure you never have multiple PODS running with the same pod ID
...
// NOTE: There are many different base32 implementations so careful when
// doing any interoperation.
...
func ParseIntBytes(id [8]byte) SnowflakeID {
	return SnowflakeID(int64(binary.BigEndian.Uint64(id[:])))
}
...
func (f SnowflakeID) Node() int64 {
	return int64(f) & podMask >> podShift
}
